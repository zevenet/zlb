...
sub modify_interface_nic    # ( $json_obj, $nic )
{
	...
	unless ( grep ( { $nic eq $_ } @system_interfaces ) and $type eq 'nic' )
	{
		my $msg = "Nic interface not found.";
		&httpErrorResponse( code => 404, desc => $desc, msg => $msg );
	}
   ...
   #not modify gateway or netmask if exists a virtual interface using this interface
	...
	if ( exists $json_obj->{ gateway } )
	{
		unless ( defined ( $json_obj->{ gateway } )
				 and &getValidFormat( 'IPv4_addr', $json_obj->{ gateway } )
				 or $json_obj->{ gateway } eq '' )
		{
	...
	if ( $json_obj->{ ip } )
	{
		if ( ( ( $json_obj->{ ip } ne $if_ref->{ addr } ) and $if_ref->{ addr } )
			 or not $if_ref->{ addr } )
		{
			require Zevenet::Net::Util;
	...
	unless ( $if_ref->{ addr } and $if_ref->{ mask } )
	{
